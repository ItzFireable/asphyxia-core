...
// Cache of pre-built hash tables:  key =
// `${gameCode}:${internalVer}:${saltHex}`
...
function buildHashTable(
    methodNames: string[], saltHex: string,
    iterCount: number): Record<string, string> {
...
export const SegaRouter = (plugins: CorePlugin[]) => {
  ...
  router.post(
      [
        '/:game/:version/:servlet/:endpoint',
        '/:game/:version/:servlet/MatchingServer/:endpoint'
      ],
      async (req: Request, res: Response) => {
        ...
        if (endpoint.toLowerCase() === 'ping') {
          return res.send(
              zlib.deflateSync(Buffer.from('{"returnCode":"1"}', 'utf8')));
        }

        let rawBody: Buffer;
        // Use pre-buffered body from express.raw() middleware if available,
        // otherwise fall back to reading the stream directly.
        ...
        const internalVer =
            plugin.SegaVersionMap ? plugin.SegaVersionMap(game, version) : -1;
        ...
        if (isHashed && internalVer !== -1) {
          ...
          if (cryptoKeysConfig && cryptoKeysConfig[String(internalVer)]) {
            const [keyHex, ivHex, saltHex, iterOverride] =
                cryptoKeysConfig[String(internalVer)];
            const iter = iterOverride ??
                (plugin.SegaIterCounts[String(internalVer)] || 0);

            if (iter > 0) {
              const cacheKey = `${game}:${internalVer}:${saltHex}`;
              if (!hashTableCache[cacheKey]) {
                Logger.info(`[SegaRouter] Building hash table for ${game} int:${
                    internalVer} (${iter} iters)...`);
                hashTableCache[cacheKey] =
                    buildHashTable(plugin.SegaMethodNames, saltHex, iter);
                Logger.info(`[SegaRouter] Hash table built, ${
                    Object.keys(hashTableCache[cacheKey]).length} entries`);
              }
              ...
              if (resolved) {
                ...
                Logger.error(`[SegaRouter] No hash match for ${game} int:${
                    internalVer} endpoint ${endpoint}`);
                return res.send(
                    zlib.deflateSync(Buffer.from('{"stat":"0"}', 'utf8')));
              }
            }
          }
        }
        ...
        if (encrypted && cryptoKey && cryptoIV) {
          try {
            const decipher =
                crypto.createDecipheriv('aes-128-cbc', cryptoKey, cryptoIV);
            decipher.setAutoPadding(true);
            bodyBuf =
                Buffer.concat([decipher.update(rawBody), decipher.final()]);
          } catch (e) {
            Logger.error(`[SegaRouter] AES decryption failed for ${endpoint}: ${
                e.message}`);
            return res.send(
                zlib.deflateSync(Buffer.from('{"stat":"0"}', 'utf8')));
          }
        ...
        if (req.headers['x-debug']) {
          ...
          // Express may auto-decompress content-encoding:deflate before we see
          // the body, so the buffer might be plain JSON already. Try inflate
          // first, fall back to raw.
          try {
            jsonBuf = zlib.inflateSync(bodyBuf);
          } catch (_) {
            try {
              jsonBuf = zlib.inflateRawSync(bodyBuf);
            } catch (_) {
              // Body arrived already decompressed by express middleware — use
              // as-is
              jsonBuf = bodyBuf;
            }
          }
        ...
        try {
          ...
          Logger.warn(
              `[SegaRouter] JSON parse failed for ${endpoint}: ${e.message}`);
          return res.status(400).end();
        }
        ...
        if (encrypted && cryptoKey && cryptoIV) {
          try {
            ...
            const padded =
                Buffer.concat([zipped, Buffer.alloc(padLen, padLen)]);
            const cipher =
                crypto.createCipheriv('aes-128-cbc', cryptoKey, cryptoIV);
            cipher.setAutoPadding(false);
            const enc = Buffer.concat([cipher.update(padded), cipher.final()]);
            return res.send(enc);
          } catch (e) {
            Logger.error(
                `[SegaRouter] AES encryption of response failed: ${e.message}`);
          }
        }
